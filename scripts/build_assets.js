...
function writeAsset(name, content) {
  fs.writeFileSync(join(HERE, "../assets/", name), content);
  console.log(`  ${name}`);
}
...
async function capture(
  browser,
  html,
  { width, height, scale = 2, selector, out, outdir, transparent = false },
) {
  ...
  await target.screenshot({
    path: resolve(HERE, "../assets/", outdir, out),
    type: "png",
...
console.log("vector");
writeAsset("75neo/logo.svg", markSVG("currentColor", { label: "75Neo" }));
for (const theme of ["light", "dark"]) {
  writeAsset(`75neo/logo-${theme}.svg`, markSVG(TOKENS[theme].primary, { label: "75Neo" }));
}
writeAsset("75neo/logo-brand.svg", brandSVG("#ffffff", BRAND_GROUND));
...
console.log("\nraster — logo");
for (const { out, outdir, mark, ground, mark_px } of [
  {
    outdir: "75neo",
    out: "logo-light.png",
    mark: TOKENS.light.primary,
    ground: null,
    mark_px: 448,
  },
  {
